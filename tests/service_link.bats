...
@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when DATABASE_URL already in use" {
  ...
  run dokku config my-app
  assert_contains "${lines[*]}" "DOKKU_SURREALDB_BLACK_URL"
  assert_success
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.surrealdb.ls:dokku-surrealdb-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps SURREALDB_DATABASE_SCHEME variable" {
  dokku config:set my-app SURREALDB_DATABASE_SCHEME=surrealdb2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app DATABASE_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
